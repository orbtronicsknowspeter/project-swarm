...
public final class g extends i {
    ...
    public final boolean e(p pVar, StringBuilder sb) {
        ...
        BigDecimal bigDecimalValueOf = BigDecimal.valueOf(uVarRange.f5921a);
        BigDecimal bigDecimalAdd = BigDecimal.valueOf(uVarRange.f5924d).subtract(bigDecimalValueOf).add(BigDecimal.ONE);
        ...
        int i = this.f5832b;
        if (iScale != 0) {
            String strSubstring = bigDecimal.setScale(Math.min(Math.max(bigDecimal.scale(), i), this.f5833c), roundingMode).toPlainString().substring(2);
            sVar.getClass();
            if (z9) {
                sb.append('.');
            }
