...
final class CircularIndeterminateAdvanceAnimatorDelegate extends IndeterminateAnimatorDelegate<ObjectAnimator> {
    ...
    private void maybeUpdateSegmentColors(int i) {
        for (int i6 = 0; i6 < 4; i6++) {
            float fractionInRange = getFractionInRange(i, DELAY_TO_FADE_IN_MS[i6], 333);
    ...
    private void updateSegmentPositions(int i) {
        ...
        for (int i6 = 0; i6 < 4; i6++) {
            activeIndicator.endFraction = (this.interpolator.getInterpolation(getFractionInRange(i, DELAY_TO_EXPAND_IN_MS[i6], 667)) * 250.0f) + activeIndicator.endFraction;
            activeIndicator.startFraction = (this.interpolator.getInterpolation(getFractionInRange(i, DELAY_TO_COLLAPSE_IN_MS[i6], 667)) * 250.0f) + activeIndicator.startFraction;
        }
