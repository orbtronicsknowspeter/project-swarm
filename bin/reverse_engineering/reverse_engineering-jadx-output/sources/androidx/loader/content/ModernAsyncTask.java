...
abstract class ModernAsyncTask<Result> {
    ...
    private final FutureTask<Result> mFuture = new FutureTask<Result>(new Callable<Result>() { // from class: androidx.loader.content.ModernAsyncTask.1
        ...
        public void done() {
            try {
                ...
                a.m("An error occurred while executing doInBackground()", e11.getCause());
            } catch (Throwable th) {
                a.m("An error occurred while executing doInBackground()", th);
            }
        }
    };
    ...
    public final void executeOnExecutor(@NonNull Executor executor) {
        ...
        if (i == 1) {
            a.i("Cannot execute task: the task is already running.");
        } else if (i != 2) {
            a.i("We should never reach this state");
        } else {
            a.i("Cannot execute task: the task has already been executed (a task can be executed only once)");
        }
    }
