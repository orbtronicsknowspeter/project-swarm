...
final class EmojiTextWatcher implements TextWatcher {
    ...
    public void onTextChanged(CharSequence charSequence, int i, int i6, int i10) {
        if (this.mEditText.isInEditMode() || shouldSkipForDisabledOrNotConfigured() || i6 > i10 || !(charSequence instanceof Spannable)) {
            return;
        }
        int loadState = EmojiCompat.get().getLoadState();
        if (loadState != 0) {
            if (loadState == 1) {
                EmojiCompat.get().process((Spannable) charSequence, i, i + i10, this.mMaxEmojiCount, this.mEmojiReplaceStrategy);
                return;
            } else if (loadState != 3) {
                return;
            }
