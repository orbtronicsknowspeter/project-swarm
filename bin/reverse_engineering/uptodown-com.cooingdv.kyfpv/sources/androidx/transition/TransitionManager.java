...
public class TransitionManager {
    ...
    public static void beginDelayedTransition(@NonNull ViewGroup viewGroup, @Nullable Transition transition) {
        ...
        Transition transitionMo224clone = transition.mo224clone();
        sceneChangeSetup(viewGroup, transitionMo224clone);
        Scene.setCurrentScene(viewGroup, null);
        sceneChangeRunTransition(viewGroup, transitionMo224clone);
    }
    ...
    private static void changeScene(Scene scene, Transition transition) {
        ...
        if (currentScene != null && currentScene.isCreatedFromLayoutResource()) {
            transitionMo224clone.setCanRemoveViews(true);
        }
        sceneChangeSetup(sceneRoot, transitionMo224clone);
        scene.enter();
        sceneChangeRunTransition(sceneRoot, transitionMo224clone);
    }
    ...
    public static TransitionSeekController controlDelayedTransition(@NonNull ViewGroup viewGroup, @NonNull Transition transition) {
        ...
        if (!transition.isSeekingSupported()) {
            com.google.gson.internal.a.p("The Transition must support seeking.");
            return null;
        }
        sPendingTransitions.add(viewGroup);
        Transition transitionMo224clone = transition.mo224clone();
        TransitionSet transitionSet = new TransitionSet();
        transitionSet.addTransition(transitionMo224clone);
