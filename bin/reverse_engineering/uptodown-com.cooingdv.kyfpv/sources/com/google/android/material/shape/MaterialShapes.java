...
public final class MaterialShapes {
    ...
    @NonNull
    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    public static Matrix createScaleMatrix(float f, float f10) {
        Matrix matrix = new Matrix();
    ...
    @NonNull
    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    public static Matrix createSkewMatrix(float f, float f10) {
        Matrix matrix = new Matrix();
        matrix.setSkew(f, f10);
        return matrix;
    }

    @NonNull
    private static RoundedPolygon customPolygon(@NonNull List<VertexAndRounding> list, int i, float f, float f10, boolean z9) {
        ArrayList arrayList = new ArrayList();
        repeatAroundCenter(list, arrayList, i, f, f10, z9);
        return RoundedPolygonKt.RoundedPolygon(toVerticesXyArray(arrayList), CornerRounding.Unrounded, toRoundingsList(arrayList), f, f10);
    }
    ...
    @NonNull
    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    public static RoundedPolygon normalize(@NonNull RoundedPolygon roundedPolygon, boolean z9, @NonNull RectF rectF) {
    ...
    private static void repeatAroundCenter(@NonNull List<VertexAndRounding> list, @NonNull List<VertexAndRounding> list2, int i, float f, float f10, boolean z9) {
        list2.clear();
        toRadial(list, f, f10);
        float f11 = (float) (6.283185307179586d / ((double) i));
        if (z9) {
            int i6 = i * 2;
            float f12 = f11 / 2.0f;
            for (int i10 = 0; i10 < i6; i10++) {
                for (int i11 = 0; i11 < list.size(); i11++) {
                    boolean z10 = i10 % 2 != 0;
                    int size = z10 ? (list.size() - 1) - i11 : i11;
                    VertexAndRounding vertexAndRounding = list.get(size);
                    if (size > 0 || !z10) {
                        list2.add(new VertexAndRounding(new PointF((i10 * f12) + (z10 ? (list.get(0).vertex.x * 2.0f) + (f12 - vertexAndRounding.vertex.x) : vertexAndRounding.vertex.x), vertexAndRounding.vertex.y), vertexAndRounding.rounding));
                    }
                }
            }
        } else {
            for (int i12 = 0; i12 < i; i12++) {
                for (VertexAndRounding vertexAndRounding2 : list) {
                    list2.add(new VertexAndRounding(new PointF((i12 * f11) + vertexAndRounding2.vertex.x, vertexAndRounding2.vertex.y), vertexAndRounding2.rounding));
                }
            }
        }
        toCartesian(list2, f, f10);
    }

    private static void toCartesian(@NonNull List<VertexAndRounding> list, float f, float f10) {
        Iterator<VertexAndRounding> it = list.iterator();
        while (it.hasNext()) {
            it.next().toCartesian(f, f10);
        }
    }

    private static void toRadial(@NonNull List<VertexAndRounding> list, float f, float f10) {
        Iterator<VertexAndRounding> it = list.iterator();
        while (it.hasNext()) {
            it.next().toRadial(f, f10);
        }
    }
    ...
    private static float[] toVerticesXyArray(@NonNull List<VertexAndRounding> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            int i6 = i * 2;
            fArr[i6] = list.get(i).vertex.x;
            fArr[i6 + 1] = list.get(i).vertex.y;
        }
        return fArr;
    }
    ...
    public static class VertexAndRounding {
        ...
        public void toCartesian(float f, float f10) {
            float fCos = (float) ((Math.cos(r0.x) * ((double) this.vertex.y)) + ((double) f));
            float fSin = (float) ((Math.sin(r0.x) * ((double) this.vertex.y)) + ((double) f10));
        ...
        /* JADX INFO: Access modifiers changed from: private */
        public void toRadial(float f, float f10) {
            this.vertex.offset(-f, -f10);
            PointF pointF = this.vertex;
    ...
    @NonNull
    @RestrictTo({RestrictTo.Scope.LIBRARY_GROUP})
    public static RoundedPolygon normalize(@NonNull RoundedPolygon roundedPolygon, boolean z9) {
        return normalize(roundedPolygon, z9, new RectF(0.0f, 0.0f, 1.0f, 1.0f));
    }
