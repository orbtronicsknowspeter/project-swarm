package j$.util.stream;

import j$.util.Spliterator;
import java.util.concurrent.atomic.AtomicLong;

/* JADX INFO: loaded from: classes2.dex */
public abstract class b8 {

    /* JADX INFO: renamed from: a, reason: collision with root package name */
    public final Spliterator f6147a;

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    public final boolean f6148b;

    /* JADX INFO: renamed from: c, reason: collision with root package name */
    public final int f6149c;

    /* JADX INFO: renamed from: d, reason: collision with root package name */
    public final long f6150d;

    /* JADX INFO: renamed from: e, reason: collision with root package name */
    public final AtomicLong f6151e;

    public abstract Spliterator b(Spliterator spliterator);

    public b8(Spliterator spliterator, long j, long j6) {
        this.f6147a = spliterator;
        this.f6148b = j6 < 0;
        this.f6150d = j6 >= 0 ? j6 : 0L;
        this.f6149c = 128;
        this.f6151e = new AtomicLong(j6 >= 0 ? j + j6 : j);
    }

    public b8(Spliterator spliterator, b8 b8Var) {
        this.f6147a = spliterator;
        this.f6148b = b8Var.f6148b;
        this.f6151e = b8Var.f6151e;
        this.f6150d = b8Var.f6150d;
        this.f6149c = b8Var.f6149c;
    }

    public final long a(long j) {
        long j6;
        boolean z9;
        long jMin;
        do {
            j6 = this.f6151e.get();
            z9 = this.f6148b;
            if (j6 != 0) {
                jMin = Math.min(j6, j);
                if (jMin <= 0) {
                    break;
                }
            } else {
                if (z9) {
                    return j;
                }
                return 0L;
            }
        } while (!this.f6151e.compareAndSet(j6, j6 - jMin));
        if (z9) {
            return Math.max(j - jMin, 0L);
        }
        long j10 = this.f6150d;
        return j6 > j10 ? Math.max(jMin - (j6 - j10), 0L) : jMin;
    }

    public final a8 f() {
        if (this.f6151e.get() > 0) {
            return a8.MAYBE_MORE;
        }
        return this.f6148b ? a8.UNLIMITED : a8.NO_MORE;
    }

    public final Spliterator trySplit() {
        Spliterator spliteratorTrySplit;
        if (this.f6151e.get() == 0 || (spliteratorTrySplit = this.f6147a.trySplit()) == null) {
            return null;
        }
        return b(spliteratorTrySplit);
    }

    public final long estimateSize() {
        return this.f6147a.estimateSize();
    }

    public final int characteristics() {
        return this.f6147a.characteristics() & (-16465);
    }

    /* JADX INFO: renamed from: trySplit, reason: collision with other method in class */
    public /* bridge */ /* synthetic */ j$.util.e1 m237trySplit() {
        return (j$.util.e1) trySplit();
    }

    /* JADX INFO: renamed from: trySplit, reason: collision with other method in class */
    public /* bridge */ /* synthetic */ j$.util.y0 m239trySplit() {
        return (j$.util.y0) trySplit();
    }

    /* JADX INFO: renamed from: trySplit, reason: collision with other method in class */
    public /* bridge */ /* synthetic */ j$.util.b1 m236trySplit() {
        return (j$.util.b1) trySplit();
    }

    /* JADX INFO: renamed from: trySplit, reason: collision with other method in class */
    public /* bridge */ /* synthetic */ j$.util.v0 m238trySplit() {
        return (j$.util.v0) trySplit();
    }
}
